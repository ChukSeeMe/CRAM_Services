...
export default function AdminPanel() {
  ...
  // Login now hits a server-side API route — password never compared client-side
        ...
        <div className="glass-panel p-10 rounded-3xl max-w-md w-full border border-white/10 text-center">
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            ...
            <button
              ...
              className="btn-premium bg-[#D18F08] text-black font-bold py-3 rounded-xl uppercase tracking-widest hover:shadow-[0_0_15px_rgba(209,143,8,0.3)] transition-all disabled:opacity-60"
            >
      ...
      <header className="border-b border-white/10 bg-[#0A0A0A] p-6 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex gap-4">
            ...
            <button onClick={() => { setIsAuthenticated(false); setPassword(''); }} className="text-gray-500 hover:text-red-500 transition-colors">
              <LogOut size={18} />
            </button>
          </div>
        </div>
                    ...
                    {blogs.map(blog => (
                      <tr key={blog.id} className="hover:bg-white/[0.02] transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-4">
                            {blog.src && <img src={blog.src} alt="" className="w-12 h-12 rounded-lg object-cover" />}
                            <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {submissions.length === 0 ? (
                ...
                submissions.map((sub, idx) => (
                  <div key={sub.id || idx} className="bg-[#0A0A0A] border border-white/5 p-6 rounded-2xl flex flex-col h-full">
                    <div className="flex justify-between items-start mb-4">
                      <span className="bg-[#111] text-[#D18F08] px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest">
                        {sub.submission_type || sub.type || 'Contact'}
                      </span>
                      <span className="text-xs text-gray-500">
                        {sub.created_at ? new Date(sub.created_at).toLocaleDateString() : (sub.timestamp ? new Date(sub.timestamp).toLocaleDateString() : '')}
                      </span>
                    </div>
                    <h3 className="text-lg font-bold text-white">{sub.name}</h3>
                    <p className="text-sm text-gray-400 mb-4">{sub.service || sub.company || ''}</p>
